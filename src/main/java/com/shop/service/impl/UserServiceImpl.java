 ...
 * @ Description：
 * @ Author     ： zlx
 * @ Date       ： Created in  2018/11/12 22:48
 * @ Modified By：
 */
...
//        }
    ...
    public ServerResponse<String> resetPassword(String passwordOld,String passwordNew,User user){
        //防止横向越权，要检验一下这个用户的旧密码，一定要指定是这个用户，因我们会查询一个 count(1)，如果不指定 id，那么结构就是 true 那 count>0;
    ...
    public ServerResponse<User> updateInformation(User user) {
        int resultCount = userMapper.checkEmailByUserId(user.getEmail(),user.getUsername());
        if(resultCount>0){
            //已存在这个邮箱地址
            return ServerResponse.createByErrorMessage("email已存在，请更换一个新的地址");
        }

        User updateUser =new User( );
        //新建一个user，不需要更改为空，防止其他项被更改
